...
import { optEmploymentPeriod, type Profile } from '@/rules/f1';
...
export default function ProfileScreen() {
  ...
  function isOpen(t: PickerTarget): boolean {
    if (!picker || picker.kind !== t.kind) return false;
    if ('index' in picker && 'index' in t) return picker.index === t.index;
    return true;
  }

  function toggle(t: PickerTarget) {
    setPicker(isOpen(t) ? null : t);
  }
  ...
  function pickerValue(): Date {
    ...
    switch (picker.kind) {
      ...
      case 'jobStart':
        return profile.employmentPeriods[picker.index]?.start ?? new Date();
      case 'jobEnd':
        return profile.employmentPeriods[picker.index]?.end ?? new Date();
      case 'cptStart':
        return profile.cptPeriods[picker.index]?.start ?? new Date();
      case 'cptEnd':
        return profile.cptPeriods[picker.index]?.end ?? new Date();
    }
  }
  ...
  function Picker({ target }: { target: PickerTarget }) {
  ...
  if (!profile) {
    return (
      <SafeAreaView style={[s.safe, s.center]}>
        <Text style={s.empty}>Set up your timeline on the Timeline tab first.</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.card}>
          <Pressable style={s.field} onPress={() => toggle({ kind: 'signature' })}>
            <Text style={s.fieldLabel}>Date last signed</Text>
            <Text style={s.fieldValue}>
              {p.lastI20SignatureDate ? fmt(p.lastI20SignatureDate) : 'Tap to set'}
            </Text>
          </Pressable>
          <Picker target={{ kind: 'signature' }} />
          {p.lastI20SignatureDate && (
            <Pressable
              style={s.remove}
              onPress={() => {
                setPicker(null);
                update({ ...p, lastI20SignatureDate: undefined });
              }}
            >
        ...
        <View style={s.card}>
          {p.employmentPeriods.map((job, i) => (
            <View key={i}>
              ...
              <Pressable style={s.field} onPress={() => toggle({ kind: 'jobStart', index: i })}>
                <Text style={s.fieldLabel}>Job {i + 1} start</Text>
                <Text style={s.fieldValue}>{fmt(job.start)}</Text>
              </Pressable>
              <Picker target={{ kind: 'jobStart', index: i }} />

              <Pressable style={s.field} onPress={() => toggle({ kind: 'jobEnd', index: i })}>
                <Text style={s.fieldLabel}>Job {i + 1} end</Text>
                <Text style={s.fieldValue}>{job.end ? fmt(job.end) : 'Still employed'}</Text>
              </Pressable>
              <Picker target={{ kind: 'jobEnd', index: i }} />

              {job.end && (
                <Pressable
                  style={s.field}
                  onPress={() => {
                    const jobs = [...p.employmentPeriods];
                    jobs[i] = { ...jobs[i], end: undefined };
                    update({ ...p, employmentPeriods: jobs });
                  }}
                >
                  <Text style={s.linkText}>Mark as still employed</Text>
                </Pressable>
              )}
        ...
        <View style={s.card}>
          {p.cptPeriods.map((c, i) => (
            <View key={i}>
              {i > 0 && <View style={s.divider} />}
              <Pressable style={s.field} onPress={() => toggle({ kind: 'cptStart', index: i })}>
                <Text style={s.fieldLabel}>CPT {i + 1} start</Text>
                <Text style={s.fieldValue}>{fmt(c.start)}</Text>
              </Pressable>
              <Picker target={{ kind: 'cptStart', index: i }} />

              <Pressable style={s.field} onPress={() => toggle({ kind: 'cptEnd', index: i })}>
                <Text style={s.fieldLabel}>CPT {i + 1} end</Text>
                <Text style={s.fieldValue}>{fmt(c.end)}</Text>
              </Pressable>
              <Picker target={{ kind: 'cptEnd', index: i }} />
